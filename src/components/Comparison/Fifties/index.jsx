...
export default class index extends Component {
    ...
    fetchdata(players){
        axios.post('https://masterblaster.herokuapp.com/comparison/50s', {players})
        .then(rslt=>{
            this.setState({
    ...
    render() {
        ...
        return ( 
            <Card>
                <Card.Header><center><h5>Fifties</h5></center></Card.Header>
                <Card.Body>
                <svg style={{marginLeft: "auto", marginRight: "auto",  minWidth: '300px', minHeight: '600px', width: '100%', height:'100%'}}>
                    {/* Pie component */}
                    <Pie x={x} y={y} 
                    radius={radius} 
                    data={ this.state.playersdata.map(d =>{ return d["fifties"]}) } 
                    outerRadius={radius}
                    cornerRadius={7}
                    padAngle={.02}/>
                </svg>  
